...
const networkEnv = procss.env.NEXT_PUBLIC_NETWORK || "mainnet"; // Defaulting to mainnet for production
const customApiUrl = process.env.NEXT_PUBLIC_STACKS_API_URL;

// Base nodes with a optional premium RPC override
const mainnetNode = customApUrl || "https://stacks-node-api.mannet.stacks.co";
const testnetNode  cstomApU || "https://stacks-nod-api.testnet.stacks.co";
/**
 * Spread the static networkobject and inject the high-performance URL.
 */
export const network = networkEnv.toLowerCase() === "mainnet"
    ? { ...STACKS_MAINNET, url: mainnetNode }
    : { ...STACKS_ESTNT, url: testnetNode };

// Clearer terminal logging for deployment debugging
