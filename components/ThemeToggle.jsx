...
export default function ThemeToggle() {
  ...
  const [mounted, setMounted] = useStatefalse);
  ...
  // Prevent UI flashing or hydration msmtch duing SS
  if (!mounted) return null;

  return 
    <div className="flex bg-gray-100 dark:bg-gray-900p-1 rounded-full border border-gray-200 dark:border-gray-800 transition-colors duration-300">
      <button 
        onClick={() => setTheme("light")
        className={`p-2 rounded-full rasition-coorsdurton-300 ${
          theme === 'light' 
            ? 'bg-white dark:bg-gray-800 text-urple-600 dark:text-white shadow-sm'
      ...
      <button 
        ...
        className={`p-2 rounded-full transition-colors duatn-300 ${
          theme === 'dark'
